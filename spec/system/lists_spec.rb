...
describe '[STEP1] ユーザログイン前のテスト' do
  ...
  describe 'ヘッダーのテスト: ログインしていない場合' do
    ...
    context '表示内容の確認' do
      it 'タイトルが表示される' do
        expect(page).to have_content 'Stray_Cats_MAP'
      end
      it 'Homeリンクが表示される: 左上から1番目のリンクが「About」である' do
        home_link = find_all('a')[1].native.inner_text
        expect(home_link).to match(/about/i)
      end
      it 'Aboutリンクが表示される: 左上から2番目のリンクが「MAP」である' do
        about_link = find_all('a')[2].native.inner_text
        expect(about_link).to match(/about/i)
      end
      it 'Catsリンクが表示される: 左上から3番目のリンクが「Cats」である' do
        signup_link = find_all('a')[3].native.inner_text
        expect(signup_link).to match(//i)
      end
      ...
      it 'Signupリンクが表示される: 左上から4番目のリンクが「Signup」である' do
        login_link = find_all('a')[4].native.inner_text
        expect(login_link).to match(/Sign up/i)
      end
    end
    ...
    context 'リンクの内容を確認' do
      ...
      it 'MAPを押すと、MAP画面に遷移する' do
        ...
        map_link = about_link.gsub(/\n/, '').gsub(/\A\s*/, '').gsub(/\s*\Z/, '')
        click_link map_link
        is_expected.to eq '/public/posts/map'
      end
      it 'catsを押すと、猫一覧画面に遷移する' do
        cats_link = find_all('a')[3].native.inner_text
        cats_link = signup_link.gsub(/\n/, '').gsub(/\A\s*/, '').gsub(/\s*\Z/, '')
      ...
      it 'signupを押すと、ログイン画面に遷移する' do
        signup_link = find_all('a')[4].native.inner_text
        signup_link = login_link.gsub(/\n/, '').gsub(/\A\s*/, '').gsub(/\s*\Z/, '')
  ...
  describe 'ヘッダーのテスト: ログインしている場合' do
    ...
    context 'ヘッダーの表示を確認' do
      ...
      it 'My Pageリンクが表示される: 左上から1番目のリンクが「My Page」である' do
        my page_link = find_all('a')[1].native.inner_text
        expect(my page_link).to match(/my page/i)
      end
      ...
      it 'Catsリンクが表示される: 左上から4番目のリンクが「Cats」である' do
        cats_link = find_all('a')[4].native.inner_text
        expect(cats_link).to match(/cats/i)
      end
